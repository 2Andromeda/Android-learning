...
public class MainActivity extends AppCompatActivity {
    ...
    Button btnInit, btnInsert, btnSelect, btnDelete, btnEdit;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnDelete.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v){
                ...
                sqlDB.close();
                Toast.makeText(getApplicationContext(), "삭제됨", 0).toString();
                btnSelect.callOnClick();
            }
        });
    ...
    public class myDBHelper extends SQLiteOpenHelper{
        ...
        @Override
        public void onCreate(SQLiteDatabase db){ //테이블을 생성하는 코딩
            db.execSQL("CREATE TABLE groupTBL (gName CHAR(20) PRIMARY KEY, gNumber INTEGER);");
        }
        @Override
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion){
            db.execSQL("DROP TABLE IF EXISTS groupTBL");
            onCreate(db);//업그레이드 할때마다 db에 제작되어있는 자료를 지우고 다시 리로딩 (onCreate)한다.
        }
    }
}